...
@IonicPage()
@Component({
  selector: 'page-strikeimpact',
  templateUrl: 'strikeimpact.html',
  providers: [Matchservice]
})
export class StrikeimpactPage {
  	...
  	for(let set of this.matchService.getNumberofSets()) {
	    ...
	    reportData = { labels: [], datasets: [] };
			...
			teampoints.forEach(function (o) {
				...
				if((['Strike','Block', 'Drop'].indexOf(o.wonMethod) > -1) || ((['Strike Error', 'Drop Error'].indexOf(o.errorMethod) > -1) && o.wonMethod == 'Opponent Error')) {
				
					let method = (o.wonMethod == 'Opponent Error' ? "Finish Errors" : "Finish Points")
					let colors = { 
						"Finish Points": ['#22cece', '#22cece','#22cece', '#22cece','#22cece', '#22cece','#22cece', '#22cece', '#22cece', '#22cece'],
						"Finish Errors": ['#FF3D67', '#FF3D67', '#FF3D67', '#FF3D67', '#FF3D67', '#FF3D67', '#FF3D67', '#FF3D67', '#FF3D67', '#FF3D67']
					}
					let errorColors = ['']
				    ...
				    let finish_condition = (method == 'Finish Points' && (user == o.wonBy || user == o.assistBy))
				    let error_condition = (method == 'Finish Errors' && user == o.errorBy)
