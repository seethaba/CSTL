// Player Statistics => Player Impact
...
@IonicPage()
@Component({
  selector: 'page-playerimpact',
  templateUrl: 'playerimpact.html',
  providers: [Matchservice]
})
export class PlayerimpactPage {
  	...
  	for(let set of this.matchService.getNumberofSets()) {
	    ...
	    reportData = { labels: [], datasets: [] };
			...
			teampoints.forEach(function (o) {
				if(o.wonMethod != 'Other') {
				
					let method = (o.wonMethod == 'Opponent Error' ? "Errors" : "Points")
					let colors = { 
				    ...
				    let finish_condition = (method == 'Points' && (user == o.wonBy || user == o.assistBy))
