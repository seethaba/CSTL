...
@IonicPage()
@Component({
  selector: 'page-serviceimpact',
  templateUrl: 'serviceimpact.html',
  providers: [Matchservice]
})
export class ServiceimpactPage {
	barChartSet1: any;
	barChartSet2: any;
	barChartSet3: any;
  	...
  	for(let set of this.matchService.getNumberofSets()) {
	    ...
	    reportData = { labels: [], datasets: [] };
			...
			teampoints.forEach(function (o) {
				if(o.wonMethod == 'Service' || (o.errorMethod == 'Service Error' && o.wonMethod == 'Opponent Error')) {
				
					let method = (o.wonMethod == 'Opponent Error' ? "Service Errors" : "Service Points")
					let colors = { 
				    ...
				    let finish_condition = (method == 'Service Points' && (user == o.wonBy || user == o.assistBy))
				    let error_condition = (method == 'Service Errors' && user == o.errorBy)
